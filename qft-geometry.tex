...
The same localization phenomenon occurs in general \cite{MR674406}:
...
\begin{equation} \label{eq:dh-integrated-over-fibers}
  Z = \int_M \frac{\omega^n}{n!} \e^{\I \alpha H}.
\end{equation}
This is the quantity which appears in the Duistermaat-Heckman formula
\eqref{eq:dh-formula}.
...
Now we replace $S \to S + \lambda Q \sigma$.
By the same arguments we used above
to prove \autoref{prop:Z-deformation-invariance},
$Z$ is independent of the parameter $\lambda$.
Taking $\lambda$ very large, we can as usual 
reduce to a neighborhood of the zero locus of $\norm{Y}^2$ i.e. to the fixed locus
of $Y$: the steepest descent method now says
\begin{equation}
  Z \sim \sum_{x_c \in M: Y(x_c) = 0} \fixme{...}
\end{equation}
