...
is a smooth function $k_t$ on $Y \times Y$, obeying\footnote{Our
...
the operator $U_t$ of ``evolution for
time $t$'' is given by
\begin{equation}
...
the heat kernel, as given e.g. in \fixme{BGV}:
as $\Delta t \to 0$ we have
\begin{equation}
  k_{\Delta t}(x,y) \sim \left( \frac{1}{4 \pi t} \right)^{\frac{\dim Y}{2}} \exp \left( - \frac{1}{4t} d(x,y)^2 \right).
\end{equation}
Substituting this above we get
\begin{align}
  k_T(y_N, y_0) &\sim \int_{Y^{N-1}} \prod_{n=1}^{N-1} \dvol_{y_n} \prod_{n=0}^{N-1}\left( \frac{1}{4 \pi \Delta t} \right)^{\frac{\dim Y}{2}} \exp \left( - \frac{1}{4 \Delta t} d(y_{n+1},y_n)^2 \right) \\
  &=  \int_{Y^{N-1}} \de \mu_N \exp \left( - \frac{\Delta t}{4} \left(\frac{d(y_{n+1},y_n)}{\Delta t}\right)^2 \right)
\end{align}
and this last expression
is the desired restriction of $S$ to polygonal paths.
\fixme{except it's missing the $R$ part which maybe
comes from first subleading term in the heat kernel asymptotics?}

When $V \neq 0$ we can make a similar formal argument.
...
\begin{equation}
  Y = \R, \qquad V = \frac{12}{\omega^2 x^2}, \qquad \omega \in \R.
\end{equation}
Then we have
\begin{equation}
