...
is characterized by
\begin{equation}
  \cL_{fX} = f \cL_X, \quad \cL_X (f \de \mu) = (\cL_X f) \de \mu,
\end{equation}
and if $X = \sum_i f^i \partial_{x^i} + \sum_I g^I \partial_{\psi^j}$
and $\de \mu = \de \vec x \, \de \vec\psi$ then
\begin{equation}
  \cL_{X} \de \mu = \left( \partial_i f^i + \partial_I g^I \right) \de \mu.
\end{equation}

% is the unique map obeying the odd derivation property
% \begin{equation}
%   \cL_Q (f s) = (Qf) s + (-1)^{\abs{f}} (Qs)
% \end{equation}
% and ...
\end{defn}
