...
{These are partial notes for a Fall 2017
course at UT Austin. They are extremely incomplete, unreliable, full
of mistakes and omissions.
Moreover, the course is now complete, but 
partway through the semester I had to stop updating the notes.
I hope to return to them, no later than the next time I teach
the course.

The latest PDF be found at
\begin{center}
\small \url{https://gauss.math.yale.edu/~an592/exports/qft-geometry.pdf}
\end{center}
...
discussed in \autoref{sec:susy-discretization}:
in order for $Z_{S^1(T)}$ to be well defined, $Y$ needs to be 
a \ti{spinnable} manifold.
...
Suppose $Y$ is even-dimensional, say $\dim Y = 2n$.
...
In this section we are going to ``prove'' the index theorem,
\autoref{thm:atiyah-singer}, using our supersymmetric quantum mechanics.
Of course, because we have been rather cavalier about infinite-dimensional
determinants, this is not a rigorous proof as it stands.
The original references for this proof are 
\cite{Alvarez-Gaume1983b,MR816738,MR87i:81237}.
...
One extension of the physical proof of the index theorem 
is worth mentioning. It was given first in \cite{Friedan1984}.
...
for twisted Dirac operators, \autoref{thm:atiyah-singer-twisted}.
...
One relevant reference here appears to be \cite{Alvarez2009}.)


\section{Some QFT generalities}

\fixme{Hilbert spaces associated to spatial slices}
\fixme{canonical quantization}


\section{QFT in 2 dimensions}

Now we move on to two-dimensional theories, i.e. theories defined
on a Riemannian $2$-manifold $X$.


\subsection{The free boson}

A simple case to start with is the theory of a single 
scalar field. This theory has
\begin{equation}
  \cC_X = \Map(X,\R)
\end{equation}
and the action
\begin{equation} \label{eq:action-2d-free-boson}
  S(\phi) = \half \int_X \norm{\de \phi}^2 \de \vol_X = \half \int_X \de \phi \wedge \star \de \phi.
\end{equation}

\begin{exercise}
Verify that $S(\phi)$ is \ti{conformally invariant}, i.e. it depends only on the conformal
class of the metric on $X$.
\end{exercise}

In principle this theory should be defined by some kind of discretization, cutoff
or the like. For a while, let us try to work formally, without being careful about
precisely how the theory is defined.

We consider the special case $X = S^1(T) \times S^1(L)$.
In this case we will see that the theory essentially decomposes into an infinite
number of copies of the $1$-dimensional ``harmonic oscillator'' we considered in
\autoref{exa:harmonic-oscillator}. Indeed, we can decompose
the field\footnote{We use $x$ as the coordinate on ``space'' $S^1(L)$
and $t$ on ``time'' $S^1(T)$.}
\begin{equation} \label{eq:fourier-expansion-2d}
  \phi(x,t) = \frac{1}{\sqrt{2L}} \sum_n a_n(t) \e^{2 \pi \I n x / L}
\end{equation}
where $a_n(t) = \overline{a_{-n}(t)}$,
and then the action \eqref{eq:action-2d-free-boson} is
\begin{equation}
  S(\phi) = \qtr \sum_{n \in \Z} \int_{0}^T \de t \, \frac{4 \pi^2 n^2}{L^2} \abs{a_n(t)}^2 + \abs{\dot{a}_n(t)}^2.
\end{equation}
Alternatively, writing $a_n(t) = b_n(t) + \I c_n(t)$ with $b_n(t)$ and $c_n(t)$ real, this is
\begin{equation}
  S(\phi) =  \sum_{n > 0} \left[ \int_{0}^T \de t \, \frac{2 \pi^2 n^2}{L^2} (b_n(t)^2 + c_n(t)^2) + \half (\dot{b}_n(t)^2 + \dot{c}_n(t)^2) \right] + \qtr \int_0^T \de t \, \dot a_0(t)^2.
\end{equation}
Thus the action is a sum of terms, each involving just one of the fields 
$a_0(t), b_n(t), c_n(t)$. Each of the $b_n(t)$ and $c_n(t)$ has an action 
like the field $x(t)$ of \autoref{exa:harmonic-oscillator}, with the 
frequency
\begin{equation}
  \omega_n = 2 \pi \frac{n}{L}.
\end{equation}
We have already computed the contribution to the partition function from such a
field: it is given by \eqref{eq:s1-z-harmonic-oscillator}.
Thus formally the fields $b_n(t)$ and $c_n(t)$ contribute to the partition function a factor
\begin{equation} \label{eq:formal-contribution-bc}
  \prod_{n=1}^\infty \frac{1}{(2 \sinh(\pi n T/L))^2} =  \prod_{n=1}^\infty \left( \frac{q^{n/2}}{1 - q^{n}} \right)^2, \qquad q = \e^{-2 \pi T / L}.
\end{equation}
As it stands, this product diverges to $0$ for any $q$, because of the factor
$\prod_{n=1}^\infty q^n$.
So our strategy of working formally
has not worked out well. On the other hand,
if we define the path integral by discretization, the situation should be
better. \fixme{explain regulation of the zero point energy}
So the regulated version of \eqref{eq:formal-contribution-bc} is
\begin{equation}
 q^{-1/12} \prod_{n=1}^\infty \frac{1}{(1-q^n)^2}
\end{equation}
This function has another name: it is $\eta(q)^{-2}$, where
$\eta(q) = q^{1/24} \prod_{n=1}^\infty (1-q^n)$ is the Dedekind eta function.

We also have the remaining field $a_0(t)$, which has to be treated differently.
\fixme{it's an IR divergence, regulate it}
Finally we get
\begin{equation}
  Z_X = \frac{V}{2 \pi \sqrt{T/L}} \eta(q)^{-2}
\end{equation}

More generally, if we consider a \ti{tilted} torus
$X = \C / (\Z \oplus \Z \tau)$ with $\tau$ in the upper
half-plane, then we get
\begin{equation} \label{eq:zx-tilted-torus}
  Z_X = \frac{V}{2 \pi \sqrt{\im \tau}} \abs{\eta(\tau)}^{-2}.
\end{equation}

\begin{exercise} Check directly that $Z_X$ given in 
\autoref{eq:zx-tilted-torus} is 
modular invariant, i.e. invariant under $\tau \to \tau + 1$
and $\tau \to -1/\tau$. \fixme{comment on this}
\end{exercise}

\fixme{comment on Polyakov formula: if we transform the metric
from $\rho_1 \abs{\de z}^2$ to $\rho_2 \abs{\de z}^2$,
$\det \Delta$ changes by $\exp \left[ \frac{1}{3 \pi} \int_X \log \frac{\rho_2}{\rho_1} \partial^2_{z \bar{z}} \log(\rho_2 \rho_1) \abs{\de z} \right]$ (cite Fay)}

\subsection{Compactified free boson}

Now let us generalize slightly: instead of the target $\R$
let us take $Y = S^1(R)$, with the same action \eqref{eq:action-2d-free-boson}.
The previous analysis now has to be modified in two ways.
First, the expansion \eqref{eq:fourier-expansion-2d} gets replaced by
\begin{equation}
  \phi(x,t) = \frac{1}{\sqrt{2L}} \sum_n a_n(t) \e^{2 \pi \I n x / L} + w x R / L
\end{equation}
where $w \in \Z$ is an integer, the \ti{winding number}.
Second, the field $a_0(t)$ appearing here is now a 
map to $S^1(R)$ rather than to $\R$. (All the other fields $a_n(t)$ are
still maps to $\R$.)

The spectrum is correspondingly modified. First, the
winding term contributes \fixme{...}

\fixme{$T$-duality}


\section{Four-dimensional field theory}

\subsection{Abelian gauge theory}

\fixme{abelian gauge theory}

\fixme{summing over bundles}

\fixme{electric/magnetic duality}

\fixme{exercise: modular property of partition function on a Riemannian manifold, ref Witten}

\fixme{coupling to background currents: Wilson line}

\fixme{coupling to dynamical currents}

\fixme{nonperturbative sectors}

\subsection{Interactions}

Now we come to an important point. The action \fixme{...} is not 
purely quadratic: it involves cubic and quartic interaction terms.
Thus, for any computation we do in this theory, we will need to
deal with the interactions.
We have done a few such computations before, in $0$-dimensional
and $1$-dimensional theories \fixme{...}, without any major difficulties arising.

Now let's consider an analogous computation in a $4$-dimensional theory:
we take a scalar field theory,
\begin{equation}
  \cC_X = \Map(X, \R),
\end{equation}
with the action
\begin{equation}
  S = \int_X \half \norm{\de \phi}^2 + \frac{m^2}{2} \phi^2 + \frac{\lambda}{4!} \phi^4
\end{equation}
This is completely 
parallel to the anharmonic oscillator which we considered in \fixme{...},
except for the crucial difference that $X$ has dimension $4$
instead of $1$.

Suppose we try to compute the simplest nontrivial observable in this theory,
the $2$-point function. Following the rules of perturbation theory as we have developed
them to this point, we would get the formal expression
\begin{equation} \label{eq:pert-formal-interacting-scalar}
  \frac{\IP{\phi(x_1) \phi(x_2)}}{Z} = D(x_1, x_2) + \frac{\lambda}{2} \int_{X} \de x' \, D(x_1,x') D(x_2,x') D(x',x') + \cdots
\end{equation}
where $D(x,y)$ is the Green's function for the deformed Laplace operator, obeying
\begin{equation}
  (-\Delta_{x} + m^2) D(x, y) = \delta(x, y)
\end{equation}
and $D(x,y) \to 0$ as $\norm{x-y} \to \infty$.
Recall \fixme{...} that for $X = \R$ and $m=0$ this would be $D(x,y) = \half \abs{x-y}$.
For $X = \R^4$ and $m=0$ on the other hand it is $D(x,y) = \frac{1}{\norm{x-y}^2}$. \fixme{constant?}
In particular, there is an evident difficulty with \eqref{eq:pert-formal-interacting-scalar}:
the quantity $D(x',x')$ appearing there is infinite.

This infinity might seem like a disaster, but it isn't: it is just a
symptom of the 
fact that we have not been particularly careful about how the theory is really defined.
Were we more careful, we might 
try to define the theory by first defining a lattice version of
it and then taking the lattice spacing to zero, as 
we did in \fixme{...}. One important effect of such a discretization is
to provide a ``UV cutoff'': roughly speaking,
the Fourier transform $\hat\phi$ cannot be supported at values
larger than the inverse lattice spacing.
In the case $X = \R^4$, 
we can model this effect by defining a new field space,
\begin{equation}
  \cC_\Lambda = \{ \phi: \R^4 \to \R, \quad \hat\phi(p) = 0 \text{ for } \norm{p} > \Lambda\}.
\end{equation}
Now, suppose that we perform our path integrals over the cutoff
field space $\cC_\Lambda$
instead of the original $\cC_{\R^4}$.
Repeating the formal development of perturbation theory, we would find that the
only change \fixme{check} is that $D(x,y)$ gets replaced by a cutoff version
$D_\Lambda(x,y)$,
defined as follows.

Recall that the original $D(x,y)$ can be written conveniently in Fourier
space, as \fixme{factors of $2\pi$?}
\begin{equation} \label{eq:propagator-fourier}
  D(x,y) = \int_{(\R^4)^*} \de p \frac{\e^{\I p \cdot (x-y)}}{\norm{p}^2 + m^2}.
\end{equation}

\begin{exercise} Check \eqref{eq:propagator-fourier}. (Hint: the
Fourier transform of $-\Delta + m^2$ is the operator of multiplication
by $\norm{p}^2 + m^2$, and the Fourier transform of the delta-function 
$\delta(0)$ is a constant function.)
\end{exercise}

In particular this gives
\begin{equation}
  D(x,x) = \int_{(\R^4)^*} \de p \frac{1}{\norm{p}^2 + m^2}
\end{equation}
which makes it obvious that it 
indeed diverges, with the divergence coming from the region of 
large $\norm{p}$, i.e. from the high-frequency modes of the field $\phi$. 
In the cutoff theory we get instead
\begin{equation}
  D_\Lambda(x,y) = \int_{\norm{p} < \Lambda} \de p \frac{\e^{\I p \cdot (x-y)}}{\norm{p}^2 + m^2}
\end{equation}
and in particular $D_\Lambda(x,x)$ is now finite.
So, by introducing the cutoff, we have at least made the
individual terms in our perturbation 
expansion \eqref{eq:pert-formal-interacting-scalar}
well defined. However, as a practical tool, this
expansion still suffers from some drawbacks. In particular,
let us consider the behavior for large 
$\Lambda$. Then we can expand \fixme{fix the constants}
\begin{equation}
  D_\Lambda(x,y) \sim \begin{cases} \Lambda^2 + \cdots & \text{ if } x = y, \\
  \frac{1}{\norm{x - y}^2} + \cdots & \text{ if } x \neq y \end{cases}
\end{equation}
Using this expansion we can study the large $\Lambda$ behavior of the
individual terms in the expansion \eqref{eq:pert-formal-interacting-scalar}.
The leading term behaves as
$\norm{x_1-x_2}^{-2}$ while the next term behaves as
$\lambda \Lambda^2$.
In particular, if $\Lambda \gg \norm{x_1-x_2}^{-1}$, then
this expansion seems to be rather ill behaved.
\fixme{...}
...
\begin{defn}[Twisted Dirac operator] The \ti{Dirac operator twisted by $E$} is 
...
% On the other hand \fixme{check} in this case 
% $\dirac^0_E$ is the $\bar\partial$ operator on sections of $E$,
% where the complex structure on $X$ is induced by the conformal structure,
% and the holomorphic structure on $E$ is induced by its connection $\nabla$.
