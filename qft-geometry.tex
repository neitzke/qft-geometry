...
obeying the formulas
\begin{equation}
  \int_V (c \de \psi) f(\psi) = c \int_V \de \psi f(\psi), \qquad \int_V \de(c \psi) f(c \psi) = \int_V \de \psi f(\psi),
\end{equation}
where $f(c \psi)$ is defined in the obvious way --- if $f(\psi) = a \psi + b$
then $f(c \psi) = c a \psi + b$.
Then prove the odd change of variables formula in one variable:
\begin{equation}
  \de (c \psi) = \frac{1}{c} \de \psi.
\end{equation}
\end{exercise}

In our case of $V = \R^{0 \vert 2}$, 
this means that up to a constant multiple, 
integration of $f(\psi^1, \psi^2)$ just means taking the 
coefficient in front of $\psi^1 \psi^2$ in the expansion of $f$.
Fixing the overall constant
amounts to specifying our precise choice of $\de \mu$.
Let us choose $\de \mu$ so that the integral is precisely
the coefficient of $\psi^1 \psi^2$.
Then applying this to \eqref{eq:2-fermion-partition-function-expanded}
gives
\begin{equation}
 Z = - \half M.  
\end{equation}
...
and an element $M \in \Sym^2(V^*) = \wedge^2((V^1)^*)$.
Then we can again write the quadratic action
\begin{equation}
  S(\psi) = \half M(\psi,\psi).
\end{equation}
Taking the fermionic integral
\begin{equation}
  Z = \int_\cC \de \mu \, \e^{-S}
\end{equation}
then gives \fixme{...}
