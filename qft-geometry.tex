...
of $N+1$ points, with spacing $\Delta t = \frac{T}{N}$; then we would have
...
\begin{equation} \label{eq:discretized-1d-measures}
 \de\mu_N = \frac{1}{(4 \pi \Delta t)^{\frac{\dim Y}{2} \times N}} \prod_{n=1}^N \dvol_Y(\phi(t_n)).
\end{equation}
...
The limit does indeed exist, which I think has been known for a 
long time when $Y = \R^n$, but is apparently much more recent
for general $Y$, see \cite{MR1698956,Baer2007}.
...
the operator $U_t = \e^{-t (-\Delta + V)}$ of ``evolution for
time $t$'' is given by
\begin{equation}
...
Here is a formal computation which gives some partial
...
\begin{equation}
  k_{\Delta t}(x,y) \sim \left( \frac{1}{4 \pi t} \right)^{\frac{\dim Y}{2}} a(y) \exp \left( - \frac{1}{4t} d(x,y)^2 \right),
\end{equation}
where $a(y)$ is some quantity built from local invariants of the metric. Substituting this above we get
\begin{align}
  k_T(y_N, y_0) &\sim \int_{Y^{N-1}} \prod_{n=1}^{N-1} \dvol_{y_n} \prod_{n=0}^{N-1}\left( \frac{1}{4 \pi \Delta t} \right)^{\frac{\dim Y}{2}} a(y_n) \exp \left( - \frac{1}{4 \Delta t} d(y_{n+1},y_n)^2 \right) \\
  &=  \int_{Y^{N-1}} \prod_{n=1}^{N-1} \dvol_{y_n} \prod_{n=0}^{N-1} \left( \frac{1}{4 \pi \Delta t} \right)^{\frac{\dim Y}{2}} a(y_n) \exp \left( - \frac{\Delta t}{4} \left(\frac{d(y_{n+1},y_n)}{\Delta t}\right)^2 \right)
\end{align}
\fixme{...}
When $V \neq 0$ we can make a similar formal argument,
using the following applied to $A = -T \Delta$, $B = T V$:
\begin{prop}[Trotter product formula]
