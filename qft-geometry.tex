...
is indeed a \ti{skew-symmetric} pairing. (Thus it would vanish
...
\end{exercise}
Thus, the same arguments we used in \fixme{...} show ---
at least formally --- that the partition function in this theory
should be independent of the metric on $Y$.
This is almost like saying that the partition function
$Z_{S^1(T)}$ is a topological
invariant of $Y$. In fact we will see that this is not quite true:
$Z_{S^1(T)}$ depends on some additional discrete structure,
namely a \ti{spin structure} on $Y$.
...
piecewise-geodesic paths from \fixme{...}, with fiber
the space of piecewise-parallel sections of $\phi^* TY$.
\fixme{is this really the best choice?}
Then we define $S \in C^\infty(\cC_{X;N})$ to be the restriction
of the full action \eqref{eq:1d-susy-action} to this space.
...
Unfortunately I don't know whether a full proof of this exists in the literature \fixme{not even 100 percent sure it's true yet}.
The same statement in a slightly different form appears in 
\cite{MR816738}, where he argues that the relevant infinite-dimensional
Pfaffian should be $\sqrt{1 - \det(H_\phi)}$ where $H_\phi$ is the
holonomy of $\phi^* TY$, and notes that in general one cannot
consistently choose this square root over the whole loop space.
...
The corresponding discretized action is
\begin{equation}
  \half \sum_{i=1}^N \left( \delta_{ab} \psi^a_i \psi^b_{i+1} + 2 \alpha \psi^1_i \psi^2_i \right). 
\end{equation}

\fixme{claim: in $N \to \infty$ limit it gives Atiyah's formula}
...
to take $g(t) = \exp(2 \pi k \frac{t}{T} R)$ 
and $k \in \Z$ is a winding number; then 
$\alpha$ is shifted by $\frac{2 \pi k}{T}$.
\fixme{...}
...
over $Y$, and 
let $S = S^0 \oplus S^1$ be the spin bundle over $Y$ as in \fixme{...}.
...
\begin{exercise} Show that $\Spin(V)$ acts on $V$ by conjugation,
and that this action gives a map $\Spin(V) \to \SO(V)$ which 
is a double cover.
\end{exercise}
...
When $n > 2$, $\Spin(n)$ is simply connected, so the 
...
\begin{example}[Spin structures on the circle] \fixme{...}
\end{example}

\begin{prop}[Spin representations] 
$\Cliff(2n)$ admits a $\Z/2\Z$-graded irreducible complex representation, $S = S^0 \oplus S^1$, where each of $S^0$ and $S^1$ has complex dimension $2^{n-1}$ and is a representation of $\Spin(V)$.
Up to equivalence $S$ is the unique irreducible representation
of $\Cliff(2n)$.
\end{prop}
...
\begin{defn}[Spinor bundles] Fix a spin manifold $X$. The spin structure is 
a principal $\Spin(n)$-bundle $Q$ over $X$.
The associated bundle $SX = Q \times_{\Spin(n)} S$ is the \ti{spinor bundle} over $X$.
\end{defn}
