...
\begin{equation}
  \cH_k = \{ \omega \in \Omega^k(X) \vert \Delta \omega = 0 \}.
\end{equation}
The equation $\Delta \omega = 0$ is a \ti{linear} equation over $\R$.
Thus the resulting $\cH_k$ is a \ti{vector space}.
If $X$ is compact, then $\cH_k$ is moreover finite-dimensional (a consequence of
