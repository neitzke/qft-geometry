...
invariant under a family of odd symmetries $Q_t$,
i.e. $Q_t S_t = 0$.
Finally, suppose
\begin{equation}
...
fixed points] \label{thm:abbv-isolated}
...
to an integral over $F$. The normal bundle $NF$
...
a $\U(1)$-equivariant $\SO(2n)$-bundle $E$. The \ti{equivariant 
Euler form} of $E$ is \fixme{check factors of $2 \pi$ and $\I$}
\begin{equation}
  \Euler(E) = \Pf\left(F + \frac{\I Y}{2 \pi}\right) \in \Omega^*(M)
\end{equation}
where $Y \in \Omega^0(\fso(E))$ is the generator of the $\U(1)$ action
and $F \in \Omega^2(\fso(E))$ is the curvature.
\end{defn}
Note that the bottom component of $\Euler(E)$ is just
$\prod_{i=1}^n k_i$ where the $k_i$ are the weights of the 
...
\begin{thm}[Atiyah-Bott-Berline-Vergne localization in general] \label{thm:abbv}
