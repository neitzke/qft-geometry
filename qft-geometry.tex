...
of the full action \eqref{eq:1d-susy-action} \fixme{...}.
...
In short, $\dirac$ gives a \ti{square root} of $-\Delta$. This is one of the
nice features of spinors: for ordinary functions, the square root of $-\Delta$
...
we can define a corresponding characteristic class of $\SO(2n)$-bundles over $X$: 
to construct it, we locally block-diagonalize the curvature 
$F = \bigoplus_i \begin{pmatrix} 0 & F_i \\ -F_i & 0 \end{pmatrix} \in \fso(2n)$
and then take the form $C(\{\frac{F_i}{2\pi}\}) \in \Omega^*(X)$.\footnote{We 
...
thus integrating appropriate combinations of them gives integers.
...
\begin{prop} If $X$ is a $4$-manifold then $\int_X p_1(X) = 3 \, {\mathrm{sign}}(X)$.
\end{prop}

\begin{exercise} Use \eqref{eq:pontr-examples} to
show that there is no orientation-preserving diffeomorphism of $\C\PP^2$ or $K3$:
these manifolds are, in some essential way, \ti{chiral}.
\end{exercise}
