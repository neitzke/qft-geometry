...
\item Show that, for fixed $m$, $Z(m, \lambda)$ admits analytic continuation to $\C \setminus \{ 0 \}$
and this continuation has an essential singularity at $\lambda = 0$.
\item Conclude that $Z(m, \lambda)$ cannot have
...
Our basic object is a vertex with $4$ half-edges attached. Such a vertex will ``carry'' a factor $-\tlambda$.
...
\end{equation}
\fixme{orbits and orbit-stabilizer theorem}
