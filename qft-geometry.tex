...
\end{equation}
and if $X = f^i \partial_{x^i} + g^I \partial_{\psi^I}$ relative to a basis of $V$, then
\begin{equation}
  \cL_{X} ( \de \vec x \, \de \vec\psi ) = \left( \partial_{x^i} f^i - \partial_{\psi^I} g^I \right) \de \mu.
\end{equation}
\end{defn}
...
(even or odd)
on a super vector space $V$ with measure $\de \mu$, 
...
\begin{pf} By direct computation: if $Q = f^i \partial_{x^i} + g^I \partial_{\psi^I}$ and $\de \mu = \de \vec{x} \de \vec{\psi}$, then
\begin{align}
  \int_V \de \mu \, Qf &= \int \de \vec{x} \, (Qf)^\rmtop \\
  &= \int \de \vec{x} \, ((\partial_{\psi^I} g^I) f + f^i \partial_{x^i} f)^\rmtop \\
  &= \int \de \vec{x} \, ((\partial_{\psi^I} g^I - \partial_{x^i} f^i) f)^\rmtop \\
  &= 0 
\end{align}
...
In each such chart, $\cC$ has base coordinates $x^i$ (even)
...
\end{equation}
This function is invariant under the odd vector field
\begin{equation}
