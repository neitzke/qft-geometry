...
spin representation: as a representation of $SO(2)^n \subset SO(2n)$,
$S$ is the tensor product of $2$-dimensional representations. Moreover, the subrepresentations
$S^0$ and $S^1$ are distinguished by the action of the diagonal $SO(2) \subset SO(2)^n$:
\fixme{...}
\end{pf}
...
Z_{S^1(T)} = \STr \e^{-TH},
\end{equation}
where $H$ is the operator $-\half \Delta$ acting on $L^2(Y, SY)$.
\end{prop}
...
which make up a copy of $Y$ inside $\cL Y$.
Restricted to these loops we have $H = \omega = 0$. It still remains to evaluate
the form $\Euler(NF)$.
