...
\begin{equation}
  \partial_t k_t(x,y) + \left(-\half \Delta_x + V(x)\right) k_t(x,y) = 0
\end{equation}
...
heat equation. The same replacement is needed in comparing the formulas we write below
...
sometimes written $U_t = \e^{-tH}$
where $H = - \half \Delta + V$.
...
\begin{equation}
  0, \frac{2 \pi^2 n^2}{R^2} \text{ (with multiplicity $2$) for $1 \le n \le \frac{N}{2}-1$}
\end{equation}
Thus the partition function on $X = S^1(T)$ is
\begin{equation}
...
\end{equation}
Now, we choose the norm \fixme{why?}
...
\begin{equation}
  \sqrt{\det A} = (\omega T) \prod_{k=1}^\infty \left( 1 + \frac{\omega^2 T^2}{4 \pi^2 k^2} \right) = 2 \sinh (\omega T/2)  
\end{equation}
as desired.

What is puzzling about this is that it is not clear why the norm
\eqref{eq:qm-norm} is the right one to choose.
...
in \fixme{...}: add fermions!
Concretely, we will make a new QFT whose configuration space is formally
\begin{equation}
  \cC = \Pi T \Map(X,Y).
\end{equation}
One immediate observation is that here we will formally have a \ti{canonical}
measure, like what we had in \S\fixme{...} before where we took $\cC = \Pi T M$.
Thus we may expect that the constructions will be a little more canonical than
...
on sections of $TY$.
\fixme{remark on integration by parts}
As before, this action is invariant under the symmetry of time translations,
which we represent by its infinitesimal action $H$, \fixme{explain this}
\begin{equation}
...
\begin{equation}
  \{Q,Q\} = 2H
\end{equation}
(which implies together they give an action of $\R^{1|1}$)
