...
This infinity might seem like a disaster, but it isn't: it is just another symptom of the 
...
it and then taking the lattice spacing to zero. One important effect of such a discretization is
to provide a ``UV cutoff'':
the Fourier transform $\hat\phi$ cannot be supported at values
larger than the inverse lattice spacing.\footnote{The lattice
also provides an ``IR cutoff,'' by discretizing the possible values of
$\hat\phi$. This effect is not needed in our present discussion: we
will compute quantities that depend only weakly on the IR cutoff,
and thus we won't include such a cutoff explicitly.
In particular, the field space we will consider
is still infinite-dimensional, while in the lattice theory it would
actually be finite-dimensional.}
In the case $X = \R^4$, 
we can model this effect by defining a new field space
\begin{equation}
  \cC_\Lambda = \{ \phi: \R^4 \to \R, \quad \hat\phi(p) = 0 \text{ for } \norm{p} > \Lambda\}
\end{equation}
So, suppose that we perform our path integrals using this cutoff version of the
field space.
