...
phrases. Suffice now to say that a quantum field theory is a
rather complicated object, with much more structure than was
at first apparent in Donaldson theory.

For aficionados, here is
the \ti{action} defining this QFT:
\begin{multline}
...
\end{itemize}
with $R$ an auxiliary $2$-dimensional vector space carrying
Hermitian structure $\delta: R \otimes \overline R \to \C$
and volume form $\eps \in \wedge^2(R)$, and
$S^\pm$ the $2$-dimensional spin representations of $\Spin(4)$.
