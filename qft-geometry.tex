...
\subsection{Quantum mechanics coupled to a vector bundle}
...
where 
\begin{itemize}
\item $S(\phi)$ is defined just as in \fixme{...}.
\item
...
\fixme{the below is still preliminary}
...
say $\dim Y = 2n$, the discretized path integral
need not be zero. Let us examine it more closely.
To write down the discretized action and measure concretely, 
we need to choose an orthonormal frame
...
i.e. for each $\phi: X \to Y$ we should choose an orthonormal
trivialization $F_\phi$
of $\phi^* TY$. Then the questions arise: does the $N \to \infty$ limit
of the discretized partition functions $Z_{X;N}$ 
exist, and if so, is it  
independent of the limiting frames $F_\phi$?
The answer is \ti{almost} yes, but there is a topological subtlety
which intervenes, as follows.

First suppose $X = S^1(T)$. Then, for each fixed $\phi$, 
...
Now, for fixed $\phi$, the integral over the fermions has the 
form
\begin{equation}
  W(\phi, F_\phi) = \int_{\Pi \Gamma(\phi^* TY)} \de \vec\psi \e^{- \half \int_{0}^T g(\psi, \nabla_t \psi)} 
\end{equation}
which as usual we define by discretization and an $N \to \infty$ limit.
Naively $W(\phi, F_\phi)$ would be independent of $F_\phi$,
but instead we have the following:

\begin{prop} If $X = S^1(T)$, then $W(\phi, F_\phi)$ exists.
...
we fix a spin structure on $Y$. This picks out a restricted class
...
fermion path integral, then this sign ambiguity does not occur.

Moreover we can \ti{calculate} $W(\phi,F_\phi)$.
This is done for example in
\cite{MR816738}:

\begin{prop} \label{prop:regularized-det-fermion}
With some regularization \fixme{explain this}
the determinant of the skew pairing $\IP{\psi, \nabla_t \psi}$
is $\det(1 - \Hol_\phi)$, where $\Hol_\phi$ is the
holonomy of $\phi^* TY$.
\end{prop}
...
of $\Hol_\phi$. Thus formally we want to compute the infinite product of these eigenvalues.
After regularization this infinite product is replaced by a function with the same zeroes,
$\prod_{i=1}^n (\sin \pi \alpha_i)^2$. \fixme{spell this out a bit more}
By direct computation this agrees with $\det(1 - \Hol_\phi)$.
\end{pf}

The Pfaffian we are after is then a square root of $\det(1 - \Hol_\phi)$.
There is a natural such square root available:
\begin{prop} \label{prop:supertrace-spin-sqrt}
If $A \in \Spin(2n)$ then 
\begin{equation}
(\STr_S A)^2 = (-1)^n \det_V (1 - A),
\end{equation}
where $V$ denotes
...
For example, when $n = 1$, \autoref{prop:supertrace-spin-sqrt} 
says $(\e^{\I \theta / 2} - \e^{-\I \theta / 2})^2 = -(1 - \e^{\I \theta}) (1 - \e^{-\I \theta})$.
More generally it follows easily from an explicit formula for the character of the
spin representation: \fixme{...}

Motivated by \autoref{prop:regularized-det-fermion} and \autoref{prop:supertrace-spin-sqrt},
we claim that in fact the integral over the fermions is
\begin{equation}
  W(\phi, F_\phi) = \STr_S \Hol_\phi.
\end{equation}
(Though strictly speaking all we have really shown is that this is a very natural square
root to pick.)
...
is a \ti{difference}
\begin{equation}
  Z_{S^1(T)} = Z(S^0) - Z(S^1)
\end{equation}
where $Z(E)$ means the partition function of the bosonic theory coupled to $E$.
What this means is that the partition function on $S^1(T)$ is the
\ti{supertrace} of the spinor heat kernel:
\begin{prop} $Z_{S^1(T)} = \STr \e^{-TH}$.
\end{prop}

This function is remarkably simple to analyze:
\begin{prop}[Supertrace is the index]
$\STr \e^{-T H} = \ind \dirac$.
\end{prop}
\begin{pf}
This just uses the fact that
$\cH = L^2(Y,SY)$ is a unitary representation of
the algebra spanned by $\dirac$ and $\Delta$.
...
\fixme{this is a little informal}
Since $[\dirac, \Delta] = 0$, $\Delta$ is central, and thus
it acts as a multiple of the identity in each irreducible
representation. So, consider an irreducible representation
$V$ in which $-\Delta = E$.
...
similar to what we did in \fixme{...}.
Indeed, at least formally, the path integral we want to do is an infinite-dimensional
version of the one given there, so we can try to apply the ABBV formula
\fixme{...}.
In this analogy the bosonic part of the action plays the role of $H$,
the fermionic part is $\omega$, $M$ is the loop space $M = \cL Y$,
and the $U(1)$ action on $M$ is by
rotation of the loops. 
The fixed points of this $U(1)$ action are thus the \ti{constant} loops,
which make up a copy of $Y$ inside $M$.
Formally applying \fixme{...} would thus lead to
\begin{equation}
  Z_{S^1(T)} = \int_M \fixme{...}
\end{equation}
...
\begin{example}
We have $p_1(X) = -\frac{1}{4\pi^2} \Tr F \wedge F$, where
$F \in \Omega^2(\fso(n))$, and $\Tr$ denotes
...
\begin{defn} If $X$ is a compact Riemannian manifold, $\hat A(X) \in \Omega^*(X)$
is the characteristic class of $TX$ associated to the symmetric function
\begin{equation}
