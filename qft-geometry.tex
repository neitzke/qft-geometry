...
in the contribution from each labeling of the diagram, we have 
to be careful about the overall sign. \fixme{explain sign rules in
...
% \footnote{This
% is not the most functorial possible definition; it would be better to
% look in \fixme{Deligne-Morgan}.}
\begin{equation}
...
\begin{equation}
    \int_V \de \mu \, f = \int_{V^0} \omega^0 \int_{V^1} \omega^1 \cdot f^1.
\end{equation}
\end{defn}
...
If $X$ is a vector field on $V$, the \ti{Lie derivative}
\begin{equation}
...
\end{equation}
and if $X = h^i \partial_{x^i} + g^I \partial_{\psi^I}$ relative to a basis of $V$, then \fixme{check sign}
\begin{equation}
  \cL_{X} ( \de \vec x \, \de \vec\psi ) = \left( \partial_{x^i} h^i + \partial_{\psi^I} g^I \right) \de \mu.
\end{equation}
\end{defn}
...
\begin{align}
  ...
  &= \int \de \vec{x} \, (h^i \partial_{x^i} f + (- \partial_{\psi^I} g^I) f)^\rmtop \\
  &= \int \de \vec{x} \, ((- \partial_{x^i} h^i - \partial_{\psi^I} g^I) f)^\rmtop \\
  &= 0 
\end{align}
...
obeying $[Q,Q] = 0$ and $Q(S) = 0$.
Finally let $\{\Psi_t\}_{t \in [0,1]}$ be a smoothly varying family 
of odd elements in $C^\infty_c(V)$, with $\Psi_0 = 0$,
and let $\{S_t\}$ be the corresponding family of deformed actions,
...
at the middle step we use $Q^2 = 0$.
\end{pf}

\autoref{prop:Z-deformation-invariance} 
is a sort of ideal version of the principle of 
\ti{invariance under $Q$-exact deformations}, which has 
strong consequences. Let us apply it in our example, taking
\begin{equation}
  Q = Q_1 + Q_2.
\end{equation}
...
Now we consider varying the function $h(x)$ which appears in 
\eqref{eq:r21-susy-action} to some family $h_t(x)$. 
...
For the action \eqref{eq:r21-susy-action}, 
$Z$ depends only on the degree of $h$ and the sign of its leading term.
\end{prop}
...
\ti{does} depend on the degree of $h$ and the sign of its leading term.
Crudely speaking, the point is that so long as the leading term of $h$
is nonzero it kills the contributions from large $\abs{x}$, 
making the field space $\cC$ effectively compact. As we vary $h$,
if there is a moment when the leading term vanishes, the
resulting loss of compactness causes a problem.
This is a prototype for problems which recur over and over in 
...
are governed by the \ti{saddle point} expansion:
\begin{prop}[Saddle point expansion in one dimension] \label{prop:saddle-expansion-1d-bosonic}
...
\begin{equation}
  Z(\lambda) \sim \sum_{x_c: h(x_c) = 0} \int \de x \de \vec\psi \, \e^{- \half h'(x_c)^2 (x-x_c)^2 - h'(x_c) \psi^1 \psi^2}
\end{equation}
...
\begin{equation}
  \frac{Z(\lambda)}{\sqrt{2\pi}} = \sum_{x_c: h(x_c) = 0} \sgn(h'(x_c)).
\end{equation}
\end{prop}
...
\subsection{A zero-dimensional sigma model}
...
\begin{equation}
  Y = \omega^{-1}(\de H).
\end{equation}
Suppose moreover that all fixed points of $Y$ are isolated points $x_c$.
Our interest is in the integral
\begin{equation}
  Z = \int_M \frac{\omega^n}{n!} \e^{\I \lambda H}. 
\end{equation}
\begin{example}
...
and thus
\begin{align}
  Z &= 2 \pi \int_0^\pi \e^{\I \lambda \cos \theta} \sin \theta \, \de \theta \\
  &= -2 \pi \int_{1}^{-1} \e^{\I \lambda z} \de z \\
  &= -2\pi \I \lambda^{-1} (\e^{\I \lambda} - \e^{-\I \lambda}) \\
  &= 4 \pi \frac{\sin \lambda}{\lambda}.
\end{align}
\end{example}
Note that this answer exhibits another localization
phenomenon: it matches what one would get
by making a quadratic approximation to the integrand 
around the critical points $x_c$
of $H$, replacing the contribution near the critical
point by a Gaussian integral in the plane. Namely each
critical point contributes
\begin{equation} \label{eq:S2-critical-contrib}
(\pm \I) \frac{2\pi}{\lambda \sqrt{\abs{\det H''(x_c)}}} \e^{\I \lambda H(x_c)}. 
\end{equation}
One difference from the saddle-point expansions
we discussed before is that we have the
...
This produces a factor \fixme{check sign}
\begin{equation}
  \e^{\frac{\pi \I}{4} (n_+ - n_-)}
\end{equation}
where $(n_+,n_-)$ is the signature of $H''$.
In our example the signature is $(2,0)$ or $(0,2)$; this 
accounts for the $\pm \I$ in \eqref{eq:S2-critical-contrib}.

The same localization phenomenon occurs in general:
\fixme{check factor $n!$}
\begin{thm}[Duistermaat-Heckman localization formula]
\begin{equation}
\int_M \frac{\omega^n}{n!} \e^{\I \lambda H} = \left( \frac{2\pi}{\lambda} \right)^{n} \sum_{x_c} \e^{\frac{\pi \I}{4} (n_+(x_c) - n_-(x_c))} \frac{\e^{\I \lambda H(x_c)}}{\sqrt{\abs{\det H''(x_c) / \omega^n}}}
\end{equation}
\end{thm}


% In short, this is another situation where the
% $\lambda \to \infty$ asymptotic expansion of the integral is 
...
\begin{equation}
 \cC = \Pi T M 
\end{equation}
i.e. the tangent bundle to $M$ where we reverse the parity of 
the fibers.
Everything we have done for super vector spaces has
a supermanifold version, obtained by appropriate patching.
I will be vague about this (if I can get by.)
\fixme{supermanifold refs: DM, Etingof, VZ...}

$\cC$ has local charts induced from the charts on $M$.
In each such chart, we identify $\cC$ with $\R^{2n \vert 2n}$,
with base coordinates $x^i$ (even)
...
\begin{equation}
  S = H + \omega = H + \omega_{ij} \psi^i \psi^j.
\end{equation}
...
\begin{equation} \label{eq:dh-integrated-over-fibers}
  Z = \int_M \frac{\omega^n}{n!} \e^{-H}.
\end{equation}
This is the quantity which appears in the Duistermaat-Heckman theorem,
except that we have analytically continued to the value $\lambda = \I$.
...
Then we have
\begin{equation}
  Q \sigma = \norm{Y}^2 + \psi^i \psi^k \partial_{x^k} Y_i
\end{equation}
and 
\begin{equation}
  Q^2 \sigma = 0
...
Now we replace $S \to S + \rho Q \sigma$.
By the same arguments we used above
to prove \autoref{prop:Z-deformation-invariance},
$Z$ is independent of the parameter $\rho$.
Taking $\rho$ very large, we expect an effective localization
to a neighborhood of the zero locus of $\norm{Y}^2$ i.e. to the fixed locus
of $Y$.
Carrying out this localization gives the Duistermaat-Heckman formula
\fixme{fill in some details here}.


% For our next example we take $\cC$ to be a \ti{supermanifold}
% rather than just a super vector space. 
% Fortunately it will be a supermanifold of a rather simple kind,
% and we will not develop the full theory of supermanifolds
% formally, but try to get by with local computations.
