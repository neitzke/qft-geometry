...
Thus altogether we get (up to terms of order $(\Delta t)^2$ which we neglect)
\begin{equation}
 k_T(y_N, y_0)  \sim  \int_{Y^{N-1}} \de \mu_N \exp \left[ \Delta t \left( - \qtr \left(\frac{d(y_{n+1},y_n)}{\Delta t}\right)^2 + \frac{R(y_n)}{3} \right) \right].
\end{equation}
