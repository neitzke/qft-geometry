...
To write down the fermion measure $\de \vec \psi$ 
we need to choose an oriented frame
...
Naively the fermionic integral would be independent of $F_\phi$,
but instead we have the following:

\begin{prop} If $X = S^1(T)$, and $\phi: X \to Y$ is fixed
along with $F_\phi$, then
the $N \to \infty$ limit of the fermion integral exists;
call it $Z_X(\phi, F_\phi)$.
This limit is invariant under small deformations of $F_\phi$, but
$Z_X(\phi, F_\phi) = - Z_X(\phi, \tau F_\phi)$.
\end{prop}
