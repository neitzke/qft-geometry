...
(\pm \I) \frac{2\pi}{\alpha} \e^{\I \alpha H(x_c)}. 
\end{equation}
In making this approximation, one difference from 
the steepest-descent expansions
...
What remains is to show that their ratio is
...
\autoref{thm:duistermaat-heckman}.

Along the way we have also proved a second version of the formula:
\begin{thm}[Duistermaat-Heckman localization formula, second version] \label{thm:duistermaat-heckman-2}
\begin{equation} \label{eq:dh-formula}
...
% The part involving $g$ cancels out here (as it must since
% the dependence on $g$ entered through our $Q$-exact deformation).
% Thus for simplicity we can choose $g$ so that $gY$ is
% a local Liouville form, i.e. $\de (gY) = \omega$.
% Then the numerator becomes $\omega^n$ and the denominator
% is the Hessian 

\fixme{you can also do this for more general
