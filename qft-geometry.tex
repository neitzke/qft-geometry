...
\end{equation}
Thus formally these fields contribute to the partition function a factor
\begin{equation}
  \prod_{n=1}^\infty \frac{1}{4 \sinh(\pi n T/L)^2}
\end{equation}
\fixme{regularization...}

We also have the extra $a_0(t)$, which has to be treated differently.

As in the 1-dimensional case, the partition function will again be expressed
in the form
\begin{equation}
  Z_{X} = \Tr_\cH \e^{-TH}
\end{equation}
where now $\cH$ is a Hilbert space associated to the $1$-manifold $S^1(L)$.
