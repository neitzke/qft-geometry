...
To write down the fermion measure $\de \vec \psi$ and 
the discretized action, we need to choose an oriented frame
...
i.e. for each $\phi: X \to Y$ we should choose an oriented 
trivialization $F_\phi$
of $\phi^* TY$. Then the questions arise: does the $N \to \infty$ limit
of $Z_{X;N}$ exist, and if so, is it  
...
for the loop group $\cL\GL^+(2n)$. In particular, it
divides up into connected components, which are a torsor for
$\pi_1(\GL^+(2n))$; this group is $\Z$ for $n=1$ and $\Z / 2\Z$
...
Unfortunately I don't know whether a full proof of this exists in the literature.
To get an idea of what's going on, though, we can look at a toy model:
a stationary loop $\phi: S^1(T) \to \R^2$. In this case $\phi^*TY$ is just
the trivial bundle with fiber $\R^2$, equipped with its standard metric.
If we choose the standard trivialization, then the discretized action is
\begin{equation}
  \half \int_{0}^T g(\psi, \nabla_t \psi) = \half \sum_{i=1}^N g_{ab} \psi^a_i (\psi^b_{i+1} - \psi^b_i) = \half \sum_{i=1}^N \delta_{ab} \psi^a_i \psi^b_{i+1}
\end{equation}
...
but changes the coordinate expression of the connection $\nabla_t$ to
\begin{equation}
  \nabla_t = \partial_t + A_t
\end{equation}
where $A = g^{-1} \de g \in \Omega^1(S^1(T), \fso(2))$. The simplest choice would be
to take $g(t) = \exp(2 \pi k \frac{t}{T} R)$ where $R \in \fso(2)$ is the standard rotation generator
and $k \in \Z$ is a winding number; then we get $A_t = \frac{2 \pi k}{T} R$, with corresponding
discretized action
\begin{equation}
  \half \sum_{i=1}^N \left( \delta_{ab} \psi^a_i \psi^b_{i+1} + \frac{2 \pi k}{N} \psi^1_i \psi^2_i \right). 
\end{equation}

\fixme{spin structure}
...
\begin{equation}
  \Spin(V) \to \SO(V),
\end{equation}
obtained concretely from the action of $\Spin(V)$ on $V \subset \Cliff(V)$
by conjugation.
\end{prop}
...
\begin{prop}[Spin representations] $\Cliff(V)$ admits a unique irreducible
graded complex representation (up to equivalence), $S(V) = S = S^0 \oplus S^1$. 
If $n = \dim V$ is odd, $S^0$ and $S^1$ are equivalent representations of $\Spin(V)$,
of dimension $2^k$ where $n = 2k+1$.
If $n = \dim V$ is even, they are inequivalent representations of $\Spin(V)$, both of dimension $2^k$
where $n = 2k$.
These representations do not descend to $\SO(V)$.
\end{prop}

\begin{example}[Spin representation in dimension $2$] The spin representation 
$S$ of $\Cliff(2)$ is $2$-dimensional. In one concrete realization of this representation, 
the generators $e_1$ and $e_2$ act by
\begin{equation}
...
The associated bundle $Q \times_{\Spin(n)} S$ is the \ti{spinor bundle}
over $X$.
\end{defn}

By abuse of notation we sometimes also use the notation $S$ for the
spinor bundle. Since $Q$ has a canonical connection induced by the
Levi-Civita connection, $S$ also has a canonical connection.
In addition, since $S$ is a \fixme{...}

\begin{defn}[Dirac operator] Fix a spin manifold $X$
with a spin structure.
The \ti{Dirac operator} is the operator
$\dirac: C^\infty(S) \to C^\infty(S)$
given by
\begin{equation}
  \dirac = \sum_{i=1}^n e_i \nabla_{e_i}
\end{equation}
where $\{e_i\}$ form an orthonormal basis for $TX$,
and $TX$ acts on $S$ via the Clifford algebra action.
\end{defn}

\begin{example}[Dirac operator on $\R^2$]
If $X = \R^2$, then $S$ is the $2$-dimensional complex trivial
bundle, and
\begin{equation}
