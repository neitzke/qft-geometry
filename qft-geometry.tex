...
\end{equation}
To define measures the necessary bit of linear algebra is:
\fixme{not obviously functorial}
...
\end{equation}
\end{defn}
An element of $\Ber V$ should be thought of as the inverse
of a volume measure.
\begin{defn}[Integration over super vector space] If $V$ is a super vector space, $\de \mu = \omega^0 \otimes \omega^1 \in \Ber V$,
and $f = f^0 \otimes f^1 \in C^\infty(V)$, then
\begin{equation}
    \int_V \de \mu \, f = \int_{V^0} (1/\omega^0) \int_{V^1} (1 / \omega^1) f^1.
\end{equation}
\end{defn}
...
This is the case where for some $h: \R \to \R$ we have
...
action $S$ is invariant under a certain \ti{odd vector field}
or \ti{supersymmetry}.

\begin{defn}[Graded derivations]
If $\cA$ is a commutative superalgebra, then a map $X: \cA \to \cA$
(either even or odd)
is a \ti{derivation} of $\cA$ if it obeys
\begin{equation}
  X(aa') = (Xa)a' + (-1)^{\abs{a} \abs{X}} a(Xa').
\end{equation}
\end{defn}

\begin{defn}[Super Lie algebra] \fixme{...}
\end{defn}
...
These form a super Lie algebra under supercommutator.
\end{defn}
...
the odd vector fields $\partial_{\psi^I}$, and these
...
\begin{equation}
  Q_1 S = Q_2 S = 0.
\end{equation}
They are also divergence-free, i.e. they preserve the
...
The existence of these supersymmetries will give us a 
...
Then the partition function $Z$ is unchanged by making the deformation
...
\begin{equation}
  Z(t) = \int_\cC \de \mu \, \e^{-(S + Q \Psi_t)}
\end{equation}
and in particular \fixme{take care about diff under integral sign}
\begin{align}
  \partial_t Z(t) &= \int_\cC \de \mu \, (Q \Psi'_t) \e^{-(S + Q \Psi_t)} \\
  &= \int_\cC \de \mu \, Q \left( \Psi'_t \e^{-(S + Q \Psi_t)} \right) \\
  &= 0
\end{align}
(at the middle step we used $Q^2 = 0$.)
\end{pf}

This \ti{invariance under $Q$-exact deformations} has 
...
\eqref{eq:r21-susy-action}. Under a variation
$\dot h(x)$, the corresponding variation of $S$
is
\begin{equation}
...
\end{equation}
Thus, if we could apply \autoref{prop:Z-deformation-invariance}
in this situation, we would conclude that
$Z$ is completely independent of the function $h(x)$.
This conclusion is too strong, and the reason is that \fixme{...}
% the variations
% $\Psi$ which we need to use are not compactly supported.
