...
of $T(V)$ by the ideal generated by the relation
...
\end{equation}
where all $v_i$ have $\IP{v_i, v_i} = \pm 1$.
\end{defn}

\begin{defn}[Spin group] $\Spin(V) = \Pin(V) \cap \Cliff^+(V)$.
\end{defn}

As a convenient notation we let $\Cliff(n)$ mean the Clifford algebra on a
vector space of dimension $n$ with a positive definite quadratic form,
and similarly $\Spin(n)$, $\Pin(n)$ etc.

\begin{exercise} Show that $\Spin(1)$ is $\Z / 2\Z$.
\end{exercise}

\begin{exercise} Show that $\Spin(2)$ is $\U(1)$.
\end{exercise}

\begin{exercise} Show that $\Spin(3)$ is $\SU(2)$.
\end{exercise}
