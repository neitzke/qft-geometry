...
structure:
\begin{fact}
When $G = SU(2)$, $\cM$ has the natural structure of a ``finite-dimensional manifold away from some singular points''
\fixme{...}
\end{fact}

Donaldson's idea was to extract interesting invariants of $X$ by studying the space $\cM$. The most direct analogue of Betti numbers is to consider the 
\ti{dimension} of $\cM$: it turns out to be 
\begin{equation}
  \dim \cM = 8k - \frac32 (\chi(X) + \sigma(X)), \qquad k = \int_X p_1(E)
\end{equation}
Here $\chi(X)$ is the Euler characteristic,
\begin{equation}
  \chi(X) = b_0(X) - b_1(X) + b_2(X) - b_3(X) + b_4(X),
\end{equation}
and $\sigma(X)$ is the signature,
\begin{equation}
  \sigma(X) = b_2^+(X) - b_2^-(X).
\end{equation}
...
In 1988, 
Witten in \cite{Witten:1988ze} found a remarkable new way of thinking about
the Donaldson invariants: he interpreted the
Riemannian $4$-manifold $X$ as
a \ti{spacetime} for a certain $4$-dimensional quantum field theory
(\ti{topologically twisted $\N=2$ supersymmetric Yang-Mills theory}),
and indentified the Donaldson invariants as
\ti{correlation functions} in that quantum field theory.
...
$\vartheta \in \R / 2 \pi \Z$, and considers ``fields''
\begin{itemize}
...
One then introduces the \ti{action}, a functional on fields:
\begin{multline}
...
interacting with our pond. (It is probably \ti{not} the
relevant description if we want to know what will happen if we shoot the 
pond with a high-intensity laser!)

Now, what Seiberg and Witten did in \cite{Seiberg:1994rs} 
can be summarized in a table: \fixme{...}
