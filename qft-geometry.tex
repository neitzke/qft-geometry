...
to quadratic, in the same sense as \fixme{...} above? For example, let's consider
the \ti{anharmonic oscillator}, which is the case $Y = \R$, 
\begin{equation}
  V(x) = \half \omega^2 x^2 + \qtr \mu x^4.
\end{equation}
Following the same pattern as \fixme{...}, we can write down \ti{Feynman rules}
for the asymptotic expansion of the partition function, or correlation functions, in 
powers of $\mu$. In fact they are formally exactly the same rules as before, just adapted
to the case where $\cC$ is an infinite-dimensional space.
