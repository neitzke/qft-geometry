...
and $k \in \Z$ is a winding number; then we get $A = \frac{2 \pi k}{T} R$, which changes the 
discretized action to
\begin{equation}
...
\begin{prop}[Spin representations] $\Cliff(n)$ admits a unique irreducible
graded complex representation (up to equivalence), $S = S^0 \oplus S^1$. 
If $n$ is odd, $S^0$ and $S^1$ are equivalent representations of $\Spin(n)$,
of dimension $2^k$ where $n = 2k+1$.
If $n$ is even, they are inequivalent representations of $\Spin(n)$, both of dimension $2^k$
where $n = 2k$.
These representations do not descend to $\SO(n)$.
\end{prop}
