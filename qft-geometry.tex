...
In the examples of \autoref{sec:0d-simple-examples} the path integrals we are computing involve 
only Gaussian integrals, albeit Gaussian integrals over unbounded numbers of variables.
Now recall from \eqref{eq:partition-function-bosonic-gaussian}
that for finite-dimensional Gaussian integrals we have
\begin{equation} \label{eq:partition-function-bosonic-gaussian-redux}
  \int_{V} \de \mu \, \e^{-\half M(x,x)} = (2\pi)^{\half \dim V} \frac{\de\mu}{\sqrt{\det M}}.
\end{equation}
