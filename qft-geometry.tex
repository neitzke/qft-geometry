...
\end{equation}
\begin{exercise} Derive the formula \eqref{eq:correlator-integral-0d}.
\end{exercise}
Using \eqref{eq:correlator-integral-0d} the integrals can be done term by term, yielding
\begin{equation} \label{eq:0d-perturbation-series}
  Z(m, \lambda) \, \text{''=''} \sqrt\frac{2\pi}{m} \left(1 - \frac18 \tlambda + \frac{35}{384} \tlambda^2 + \cdots + (1390.1 \dots) \tlambda^{10} + \cdots \right), \qquad \tlambda = \frac{\lambda}{m^2}.
\end{equation}

\begin{exercise} Use \eqref{eq:correlator-integral-0d} to verify the given terms of
\eqref{eq:0d-perturbation-series}.
\end{exercise}
...
\item Show that, for fixed $m$, $Z(\lambda)$ admits analytic continuation to $\C \setminus \{ 0 \}$
and this continuation has an essential singularity at $\lambda = 0$.
\item Conclude that $Z(\lambda)$ cannot have
...
gives an asymptotic series for $Z(m,\lambda)$ as $\lambda \to 0^+$ for fixed $m$.
\end{prop}
...
The key formula \eqref{eq:correlator-integral-0d} admits a neat combinatorial interpretation,
which begins with the observation:
...
Now we can revisit \eqref{eq:dubious-exchange}, and think of it in the following way.
\fixme{...}
