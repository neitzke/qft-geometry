...
\fixme{try to comment about coordinate invariance?}
...
The $Q_1$, $Q_2$ are also divergence-free, i.e. they preserve the
integration measure $\de \mu = \de x \, \de \vec\psi$,
in the following sense.

\begin{defn}[Lie derivative along super vector field] If $V$
is a super vector space, $Q$ an odd vector field, and
$\de \mu \in \Ber V^*$, the \ti{Lie derivative} of $\de \mu$
along $Q$ is \fixme{...}
\end{defn}
...
% \end{exercise}
we conclude that $Z$ is unchanged by compactly supported
variations of $h$.
...
The strategy we follow is \fixme{roughly!} to deform $h(x) \to \lambda h(x)$ and take
$\lambda \to \infty$.
