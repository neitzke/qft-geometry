...
similarly, with the additional rule that every component of the diagram
...
\end{equation}
and the Feynman rules involve half-edges decorated by labels $i \in \{1, \dots, N\}$:
...
\end{equation}
The Feynman rules for this theory are shown below:
...
we can actually compute $S_\eff(x)$ explicitly: it is of the form
\begin{equation}
  S_\eff(x) = \frac{m_\eff}{2} x^2 + \sum_{k \ge 3} \lambda_k x^k
\end{equation}
with
\begin{equation} \label{eq:seff-terms}
  m_\eff = m + \frac{\mu}{2M}, \qquad \lambda_k = \begin{cases} 0 & \text{ for $k$ odd,} \\ - \left( - \frac{\mu}{M} \right)^{k/2} \frac{1}{2^{k/2+2} k} & \text{ for $k$ even.} \end{cases} 
\end{equation}

\begin{exercise} Use the effective action $S_\eff(x)$ to compute $\frac{\IP{x^4}}{Z}$ up to order $\mu^2$.
Note that it is a lot easier than using the original action $S(x,y)$.
\end{exercise}

\begin{exercise} Derive the formula \eqref{eq:seff-terms}.
\end{exercise}

We can also compute a Feynman-diagram expansion for $S_\eff$: \fixme{...}
Thus, diagrammatically speaking, integrating out to pass from $S$ 
to $S_\eff$ amounts to ``collapsing'' parts of the Feynman diagrams
involving only the dashed $y$ lines into the new effective vertices.
