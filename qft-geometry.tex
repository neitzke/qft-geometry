...
following a calculation given in
\cite{MR87i:81237,MR816738}:

\begin{prop} \label{prop:regularized-det-fermion}
Relative to some choice of metric in $\Gamma(\phi^* TY)$,
...
$\e^{2 \pi \I k t / T}$.)
Thus formally we want to compute the infinite product
\begin{equation}
  \prod_{k \in \Z} \prod_{i=1}^n \frac{2 \pi \I}{T} \left( k + \frac{\alpha_i}{2\pi} \right)\left( k - \frac{\alpha_i}{2\pi} \right).
\end{equation}
This infinite product has a double zero at $\alpha_i = 2 \pi k$, for any $k$.
After regularization \fixme{choice of metric enters here} 
it is replaced by a function with the same zeroes,
...
\end{equation}
where $H = -\half \Delta$ acting on $\cH = L^2(Y, SY)$.
\end{prop}
...
\begin{equation}
  \Euler(NF) = \prod_{i=1}^n \frac{\sin \pi R_i}{\pi R_i} = \hat A(Y)^{-1}
\end{equation}
where $\hat A(Y)$ was defined in \autoref{def:ahat}.
