...
better. Indeed, suppose that we regulate \fixme{...}

So the regulated version of \eqref{eq:formal-contribution-bc} is
\begin{equation}
 q^{-1/12} \prod_{n=1}^\infty \frac{1}{(1-q^n)^2} = \eta(q)^{-2}
\end{equation}
Here $\eta(q)$ is the Dedekind eta function.

We also have the remaining field $a_0(t)$, which has to be treated differently.
\fixme{it's an IR divergence}
