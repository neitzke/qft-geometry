...
notation here: suppose $A = A_i^a T_a \de x^i$, with $T_a$ a basis for $\fg$, and define the structure constants
$f_{ab}^c$ by $[T_a, T_b] = f_{ab}^c T_c$; then $A \wedge A = \half f_{ab}^c T_c A_i^a A_j^b \de x^i \wedge \de x^j$.
