...
\subsection{Perturbation theory} \label{sec:perturbation-theory-0d}
...
\begin{equation} \label{eq:correlation-hamiltonian}
	\IP{\cO_1(t_1) \cO_2(t_2) \cdots \cO_n(t_n)}_{S^1(T)} = \Tr_{\cH} \e^{-t_1 H} \hat\cO_1 \e^{-(t_2 -t_1) H} \hat\cO_2 \cdots \hat\cO_n \e^{-(T-t_n)}.
\end{equation}
...
to quadratic, in the same sense as \autoref{sec:perturbation-theory-0d} above? 
For example, let's consider
the \ti{anharmonic oscillator}, which is the case $Y = \R$, 
\begin{equation}
  V(x) = \half \omega^2 x^2 + \frac{\lambda}{4!} x^4.
\end{equation}

Let us consider the case $X = S^1(T)$.
Following the same pattern as \autoref{sec:perturbation-theory-0d}, 
we can write down 
\ti{Feynman rules} for the asymptotic expansion of the 
partition function, or correlation functions, in 
powers of $\lambda$. In fact they are formally exactly the same rules as before, adapted
to the case where $\cC$ is an infinite-dimensional vector space.

Thus to compute $Z/Z_0$ we sum over ``bubble'' diagrams with 
$4$-valent vertices.
...
\end{equation}
Then, in parallel with \fixme{...} we have
...
terms in \eqref{eq:Z-1d-pert-first}. \fixme{hard?} Use them to compute
...
provided that $\Map(X,Y)$ is oriented, 
here we have (at least formally) a \ti{canonical}
measure of integration, like what we had in \autoref{sec:0d-sigma-model-localization} 
...
subtler topological questions come into play.
\begin{exercise} Write out the discretized action $S_N$
for a general even-dimensional $Y$, and show that $Z_{S^1(T);N} = 0$ 
for any $N$ if $w_2(Y) \neq 0$. \fixme{check! is it really correct?}
\end{exercise}
...
over $Y$, and 
let $S^\pm$ be the spin bundles over $Y$ as in \fixme{...}.
We consider the Dirac operators $\dirac^\pm: S^\pm \to S^\mp$
and the corresponding Laplace operators $\Delta: S^\pm \to S^\pm$.
Recall that $\dirac^+$ is adjoint to $\dirac^-$.
Then we can define a spinorial analog of the heat equation,
just like \eqref{eq:heat-equation} except that we use
the operator
\begin{equation}
  H = [\dirac^+,\dirac^-] = - \Delta
\end{equation}
acting on $L^2(Y, S)$ where $S = S^+ \oplus S^-$.

Here, in contrast with \fixme{...}, we do \ti{not} perturb
by a potential function $V$, nor do we add a multiple of the
scalar curvature.
The $H$ we choose is singled out
...
coming from the fact that the heat kernel is not just 
a function. Thus is would not quite make sense to identify
$Z_{[0,T]_{y_0}^{y_1}}$ with the heat kernel. Instead we 
have:

\begin{prop} $Z_{[0,T]_{y_0}^{y_1}}$ is the
contraction between the heat kernel $k_T(y_0,y_1)$ 
and the map $S_{y_1} \to S_{y_0}$ given by parallel transport
