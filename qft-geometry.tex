...
Let $\cC$ be a finite-dimensional real vector space with positive definite inner product
$\IP{\cdot,\cdot}$, and two elements
\begin{equation}
...
\end{equation}
The Feynman rules now assign a vector in $(V^*)^{\otimes k}$ to any diagram element
with $k$ open half-edges, as below:
\insfig{qft-geometry-14}{0.9}
When we put two half-edges together to make an edge, we use the inverse of the pairing
$\IP{\cdot,\cdot}$ to contract the corresponding slots of the tensors.
...
\begin{equation}
  S_\eff(x) \sim \frac{m_\eff}{2} x^2 + \sum_{k \ge 3} \lambda_k x^k
\end{equation}
with
...
\begin{exercise} Derive the formula \eqref{eq:seff-terms}. \fixme{maybe hard?}
\end{exercise}
...
with the exterior algebra on $\R^2$; the even part $\cO^0(\cC)$ is spanned
by $1$ and $\psi^1 \psi^2$, the odd part $\cO^1(\cC)$ by $\psi^1$ and $\psi^2$.)
...
\begin{equation}
  Z = \int_\cC \de\mu \, \e^{-S}.
\end{equation}
For this we need some notion of a ``measure of integration'' $\de \mu$.
...
\end{equation}
\end{defn}
Our measure will just be an element of $\Ber V$.
We may think of it as the \ti{ratio} of volume forms $\omega^0$, $\omega^1$
on the two pieces of $V$. Then the integral is
\fixme{...}
