...
\item Show that, for fixed $m$, $Z(m, \lambda)$ admits analytic continuation to 
\fixme{a branched cover of?} $\C \setminus \{ 0 \}$
...
Actually, it turns out that in this particular theory $Z(m,\lambda)$ can be computed in closed form, e.g. by Mathematica:
\begin{equation}
...
More generally, 
\begin{prop}[Symmetries and correlation functions]
