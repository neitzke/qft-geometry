...
\begin{pf}
When $n = 1$, \autoref{prop:supertrace-spin-sqrt} 
says 
\begin{equation}
(\e^{\I \theta / 2} - \e^{-\I \theta / 2})^2 = -(1 - \e^{\I \theta}) (1 - \e^{-\I \theta}).  
\end{equation}
More generally it follows easily from an explicit formula for the character of the
spin representation: indeed, as a representation of $SO(2)^n \subset SO(2n)$,
$S$ is the tensor product of $2$-dimensional representations; $S^0$ is \fixme{...}
\end{pf}
...
is the supertrace of spinor heat flow] $Z_{S^1(T)} = \STr \e^{-TH}$.
\end{prop}
