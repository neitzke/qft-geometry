...
\begin{align}
  S(\phi) &= \int_X \dvol_X \left(\qtr g(\dot\phi,\dot\phi) + V(\phi) - \frac13 R(\phi) \right) \\
  &= \int_X \de t \sqrt{\eta_{tt}} \left(\qtr g_{ij}(\phi) \dot\phi^i \dot\phi^j \eta^{tt} + V(\phi) - \frac13 R(\phi)\right)
\end{align}
...
\begin{equation}
    S(\phi) = \int_X \de t \, \left( \qtr g_{ij}(\phi) \dot\phi^i \dot\phi^j + V(\phi) - \frac13 R(\phi) \right).
\end{equation}
...
\begin{equation}
  k_{\Delta t}(x,y) \sim \left( \frac{1}{4 \pi \Delta t} \right)^{\frac{\dim Y}{2}} \exp \left( - \frac{1}{4 \Delta t} d(x,y)^2 \right).
\end{equation}
The first-order correction in $d(x,y)$ can be extracted from \cite{Berline2004}: it is of the form
\begin{equation}
  k_{\Delta t}(x,y) \sim \left( \frac{1}{4 \pi \Delta t} \right)^{\frac{\dim Y}{2}} \exp \left( - \frac{1}{4 \Delta t} d(x,y)^2 \right) \left( 1 + \frac{1}{12} \Ric_x(x-y,x-y) + \cdots \right).
\end{equation}
...
\begin{equation}
  k_{\Delta t}(x,x) \sim \left( \frac{1}{4 \pi \Delta t} \right)^{\frac{\dim Y}{2}} \exp \left( - \frac{1}{4 \Delta t} d(x,x)^2 \right) \left( 1 + \frac16 R(x) \Delta t + \cdots \right)
\end{equation}
Substituting these in \eqref{eq:kernel-chopup} we get
\begin{multline}
  k_T(y_N, y_0) \sim \int_{Y^{N-1}} \prod_{n=1}^{N-1} \dvol_{y_n} \prod_{n=0}^{N-1}\left( \frac{1}{4 \pi \Delta t} \right)^{\frac{\dim Y}{2}} \times \\ 
  \exp \left( - \frac{1}{4 \Delta t} d(y_{n+1},y_n)^2 \right) \left(1 + \frac16 R(y_n) \Delta t + \frac{1}{12} \Ric_{y_n}(y_{n+1}-y_n,y_{n+1}-y_n) + \cdots \right)
\end{multline}
The last term is a bit tricky to deal with, but under the integral it can be replaced
by $\frac16 R(y_n) \Delta t$. \fixme{explain how} 
Thus altogether we get
\begin{equation}
 k_T(y_N, y_0)  \sim  \int_{Y^{N-1}} \de \mu_N \exp \left[ \Delta t \left( - \qtr \left(\frac{d(y_{n+1},y_n)}{\Delta t}\right)^2 + \frac{R(y_n)}{3} \right) + \cdots \right].
\end{equation}
...
into an operator on $L^2(Y)$, albeit
...
When $Y = \R$, use the metric to identify $Y$ with $T^* Y$
coordinatized by $(x,p)$. Then the function $F = p$
induces (in the above sense) the operator $\partial_x$
...
After all, naively we would have (where $\leftrightarrow$ means
``is the heat kernel for'')
\begin{equation}
  \e^{-t_3 H} \hat{p} \e^{-t_2 H} \hat{x} \e^{-t_1 H} \leftrightarrow \int_{\cC_{[0,T]}} \de \phi \, \phi'(t_1 + t_2) \phi(t_1) \e^{-S(\phi)}
\end{equation}
\begin{equation}
  \e^{-t_3 H} \hat{x} \e^{-t_2 H} \hat{p} \e^{-t_1 H} \leftrightarrow \int_{\cC_{[0,T]}} \de \phi \, \phi(t_1 + t_2) \phi'(t_1) \e^{-S(\phi)}
\end{equation}
Now in the limit $t_2 \to 0$ these two path integrals 
both formally limit to the same object,
\begin{equation}
\int_{\cC_{[0,T]}} \de \phi \, \phi(t_1) \phi'(t_1) \e^{-S(\phi)}
\end{equation}
but which operator is this the heat kernel for?
...
insertions the $y_2$-dependent part is either
\begin{equation}
  \frac{1}{\Delta t} \int \de y_2 \, (y_2 - y_1) y_2 \, \e^{-F(y)}, \qquad
  \frac{1}{\Delta t} \int \de y_1 \de y_2 \de y_3 \, (y_3 - y_2) y_2 \, \e^{-F(y)}
\end{equation}
where
\begin{equation}
  F(y) = [(y_1 - y_2)^2 + (y_3 - y_2)^2] / (4 \Delta t).
\end{equation}
Finally, these two integrals are not the same:
by integration by parts one shows that they differ by
\fixme{check factors}
\begin{equation}
  \int \de y_1 \de y_2 \de y_3 \, \e^{-F(y)}.
\end{equation}
Note this difference survives in the limit 
$\Delta t \to 0$!

But the latter is just what we get from an insertion
...
\fixme{time translations}

\fixme{isometries of $Y$}
...
\fixme{effective field theory in a coupled system}
